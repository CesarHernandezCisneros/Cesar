...
class MapitaActivity : AppCompatActivity() , OnMapReadyCallback, PermissionsListener {
    ...
    //Invocamos los metodos del menu
    override fun onCreateOptionsMenu(menu: Menu?): Boolean {
        //Este metodo sirve para invocar el menu que acabamos de crear y lo coloca en la toolbar
        menuInflater.inflate(R.menu.menusito,menu)
        return super.onCreateOptionsMenu(menu)
    }
//Invocamos el metodo que nos ayuda a programar los eventos de tipo touch del menu
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when(item.itemId){
            R.id.accion_todos->{
                //Esta seria la opcion a ejecutarse para el caso de el primer item
            var i=Intent(applicationContext,MapitaTodos::class.java)
                startActivity(i)
            }
            R.id.accion_uno->{
                //Esta seria la opcion a ejecutarse para el segundo item del menu
            }
            else ->{
                //El default
            }
    ...
    inner  class LocationChangeListeningActivityLocationCallback internal constructor(activity: MapitaActivity?):
        ...
        override fun onSuccess(result: LocationEngineResult){
            ...
            if(activity!=null){
                ...
                //que usuario es el que se va a actualizar


                //Aqui en esta árte vamos a invocar el servicio REST donde nosotros vamos a estar
                //mandando al back-end con la informacion de la ultima localizacion
                //Para poder hacerlo tinees que llevar lo siguiente
                ...
                //Lo asignamos al usuario, El usuario debe de estar REGISTRADO Y GUARDADO
                //LOCALMENTE EN SharedPreferences
                var usuario=JSONObject();
                usuario.put("email","ces.hernandez.cisneros@gmail.com")
